  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const jogMap: Record<string, Jogador> = {};
        (jogs ?? []).forEach((j) => (jogMap[j.id] = j as Jogador));
        setJogadores(jogMap);
    ...
    <div className="space-y-6">
      ...
      {!extrasVisiveis && bolaoAtivo && (
        <div className="rounded-xl border border-border bg-card px-4 py-3 text-sm text-muted-foreground">
          Os palpites especiais (artilheiro e campeã) ficarão visíveis aqui após{" "}
          <span className="font-medium">13/06 às 15h (Brasília)</span>.
        </div>
      )}
