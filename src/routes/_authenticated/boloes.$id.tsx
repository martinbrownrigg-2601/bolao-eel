...
import { Loader2, Copy, Check, Users, Trophy, ArrowLeft } from "lucide-react";
...
type Bolao = {
  ...
  codigo_convite: string; is_publico: boolean; criador_id: string;
};
...
function BolaoDetalhe() {
  ...
  const [ranking, setRanking] = useState<{ uid: string; nome: string; pontos: number; jogos: number }[]>([]);
    ...
    (async () => {
      try {
        ...
        const { data: ms } = await supabase
          .from("membros_bolao").select("usuario_id,entrou_em").eq("bolao_id", id);
        ...
        if (ids.length > 0) {
          const [{ data: ps }, { data: pts }] = await Promise.all([
            ...
            supabase.from("v_pontuacao_usuario").select("usuario_id,pontos_total,jogos_pontuados").in("usuario_id", ids),
          ]);
  ...
  if (erro || !bolao) {
    return (
      <div className="space-y-4">
        <Link to="/boloes" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Compartilhe o código <span className="font-mono font-bold text-foreground">{bolao.codigo_convite}</span> ou o link abaixo.
        </p>
      ...
      <section>
        <h2 className="mb-3 inline-flex items-center gap-2 text-sm font-semibold uppercase tracking-widest text-muted-foreground">
          <Trophy className="h-4 w-4" /> Ranking ({ranking.length} {ranking.length === 1 ? "membro" : "membros"})
        </h2>
        <ul className="grid gap-2">
          {ranking.map((r, i) => (
            <li key={r.uid} className="flex items-center justify-between rounded-lg border border-border bg-card px-4 py-3">
              <div className="flex items-center gap-3">
                <span className={`grid h-7 w-7 place-items-center rounded-md text-xs font-bold ${i === 0 ? "bg-accent text-accent-foreground" : "bg-secondary text-foreground"}`}>
                  {i + 1}
                </span>
                <div>
              ...
              <div className="text-lg font-bold text-primary">{r.pontos} <span className="text-xs font-normal text-muted-foreground">pts</span></div>
            </li>
          ))}
        </ul>
      </section>
